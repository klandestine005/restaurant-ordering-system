...
// Validasi Gatekeeper: pastikan yang masuk adalah akun employee dengan role kitchen
...
// Tangani aksi tombol "Done"
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'complete_order') {
    $order_to_complete = $conn->real_escape_string($_POST['order_number']);
    
    // Update status pesanan di database menjadi Done
    $update_query = "UPDATE Orders SET status = 'Done' WHERE order_number = '$order_to_complete'";
    $conn->query($update_query);
    
    // Muat ulang halaman untuk memperbarui antrean aktif
    header("Location: dashboard_kitchen.php");
    exit();
}

// Tangani fitur Search
// Ambil keyword pencarian dari URL (GET)
$search_keyword = "";
$search_condition = "WHERE status = 'On Progress'"; // Hanya antrean aktif

if (isset($_GET['search']) && trim($_GET['search']) !== '') {
    $search_keyword = $conn->real_escape_string(trim($_GET['search']));
    // Cari berdasarkan Nomor Order, Nomor Meja, atau Nama Pelanggan
...
<head>
    ...
    <title>Kitchen Orders - Orange Restaurant</title>
...
<input type="text" name="search" value="<?php echo htmlspecialchars($search_keyword); ?>" placeholder="Search order, number tables, or customer name" autocomplete="off">
        </form>
        ...
        <div class="orders-grid">
            ...
            if ($result && $result->num_rows > 0) {
                while ($order = $result->fetch_assoc()) {
                    ...
                    $time_display = date("d/m/Y - H:i:s", strtotime($order['date']));
                    ?>
                    
                    <div class="kitchen-order-card">
                        <div class="kitchen-card-header">
                            <p><strong>Order Number:</strong> <?php echo $short_id; ?></p>
                            <p class="order-date"><strong>Date - Time:</strong> <?php echo $time_display; ?></p>
                        ...
                        <div class="kitchen-card-footer">
                            <form action="" method="POST">
                                ...
                                <button type="submit" class="btn-kitchen-done">Done</button>
                            </form>
                        </div>
                    </div>
                ...
                echo '<p class="no-data-msg">There are currently no active order queues..</p>';
            }
            ?>
        </div>
    </main>
